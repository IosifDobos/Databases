...
-- 5 marks
ALTER TABLE Product Add markup NUMBER(2,2);
...
-- 5 marks
UPDATE Product SET markup = (retailPrice - costPrice) / 100;
--3. Add a constraint to the markup column so that it cannot contain null
-- 5 marks
ALTER TABLE Product MODIFY markup NOT NULL;
--4. The column drugnondrug on the product table is used to indicate 
--that a product is a drug or not a drug. 
-- If the value of this column is currently null set it to be 0
-- 5 marks
UPDATE Product SET drugnondrug = 0 WHERE drugnondrug IS NULL;

--5.
--Remove the column retailprice from the product table.
-- 5 marks
ALTER TABLE Product DROP COLUMN retailPrice; 
--6. Add a constraint to the product table to ensure that the column 
--drugnondrug can only take values 0 or 1 and that it cannot be null.
-- 10 marks
ALTER TABLE Product ADD CONSTRAINT chk_drugnondrug 
CHECK(drugnondrug = 0 OR drugnondrug = 1);

ALTER TABLE Product
MODIFY drugnondrug NOT NULL;
--7. Update the dosage of all prescriptions of the product Calpol to be 
--50 using a sub-query. You should ensure case sensitivity is not an issue.
-- 10 marks
UPDATE PrescriptionItem 
SET preDosage = 50 
WHERE stockCode = ( SELECT stockCode from Product WHERE stockDescription = 'Calpol');
--8.
--Delete all prescriptions and prescription items for customer Danny. 
--Use sub-queries to achieve this. You should ensure case sensitivity is not an issue.
-- 15 marks
DELETE FROM PrescriptionItem WHERE prescriptionID = (SELECT prescriptionID FROM Prescription
    WHERE custID = (SELECT custID FROM Customer WHERE custName = 'Danny'));
DELETE FROM Prescription WHERE custID = (SELECT custID FROM Customer WHERE custName = 'Danny');
...
ALTER TABLE Product;
UPDATE Product 
SET stockDescription = 'Lynx' 
    WHERE stockCOde = (SELECT  stockCode FROM Product WHERE stockDescription = 'Umbrella');
--10.
--Delete the product lYNX from the database. Think carefully about what this involves 
--(drug sales and non drug sales). 
--You should ensure case sensitivity is not an issue.
-- 15 marks
DELETE FROM NonDrugSale WHERE stockCode = (SELECT stockCode FROM Product WHERE stockDecription = 'Lynx');
...
SET notes = 
CASE
    WHEN docID = (SELECT docID FROM Doctor WHERE docName = 'Lindsey') OR 
         docID = (SELECT docID FROM Doctor WHERE docName = 'Jack') THEN 'Verify Instruction'
    WHEN docID = (SELECT docID FROM Doctor WHERE docName = 'Michael') THEN 'Check with customer'
